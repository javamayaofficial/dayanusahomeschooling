...
from app.models.enums import LearningTrack, PaketLevel, StudentStatus, UserRole, db_enum
class User(Base, UUIDMixin, TimestampMixin):
    __tablename__="users"
    email: Mapped[str]=mapped_column(String(255), unique=True, index=True, nullable=False)
    password_hash: Mapped[str]=mapped_column(String(255), nullable=False)
    full_name: Mapped[str]=mapped_column(String(150), nullable=False)
    role: Mapped[UserRole]=mapped_column(db_enum(UserRole), nullable=False, index=True)
...
class StudentProfile(Base, UUIDMixin, TimestampMixin):
    ...
    paket: Mapped[PaketLevel|None]=mapped_column(db_enum(PaketLevel))
    track: Mapped[LearningTrack]=mapped_column(db_enum(LearningTrack), default=LearningTrack.PKBM_PLUS_SOFTSKILL)
    nis: Mapped[str|None]=mapped_column(String(30), unique=True); enrollment_date: Mapped[date|None]=mapped_column(Date)
    status: Mapped[StudentStatus]=mapped_column(db_enum(StudentStatus), default=StudentStatus.ACTIVE)

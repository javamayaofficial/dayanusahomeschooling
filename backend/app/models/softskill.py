...
from app.models.enums import ContentType, SkillLevel, SoftSkillCategory, db_enum
class SoftSkillClass(Base, UUIDMixin, TimestampMixin):
    __tablename__="soft_skill_classes"
    title: Mapped[str]=mapped_column(String(200), nullable=False)
    category: Mapped[SoftSkillCategory]=mapped_column(db_enum(SoftSkillCategory), index=True)
    description: Mapped[str|None]=mapped_column(Text); skkni_code: Mapped[str|None]=mapped_column(String(100))
    level: Mapped[SkillLevel]=mapped_column(db_enum(SkillLevel), default=SkillLevel.BEGINNER)
...
class SkillLesson(Base, UUIDMixin, TimestampMixin):
    ...
    title: Mapped[str]=mapped_column(String(200), nullable=False)
    content_type: Mapped[ContentType]=mapped_column(db_enum(ContentType), default=ContentType.VIDEO)

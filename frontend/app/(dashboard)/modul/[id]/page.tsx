...
export default function ModuleDetailPage() {
  ...
  useEffect(()=>{ if (m?.lessons.length && !activeId) setActiveId(m.lessons[0]?.id ?? null); },[m,activeId]);
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="rounded-[28px] border border-navy-100 bg-white p-4 shadow-soft">
        <div className="px-2 pb-3">
          ...
          <p className="mt-1 text-sm text-navy-600">{m.lessons.length} materi tersedia</p>
        </div>
